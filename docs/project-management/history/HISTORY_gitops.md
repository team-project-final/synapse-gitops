...
## 2026-06-11 (라이브 윈도우) — dev 9/9 + staging 7/7 달성 · ES nori 검증 · 24h 소크 진입

### 무엇을 했는지
- **bring-up EKS ES 실기동 4결함 수정**(PR #180): fsGroup 누락(EBS root 소유→node.lock CrashLoop)·ES/schema-registry standalone App 미배포·gp3 SC 순서·es-reindex 레이스. + Git Bash kubectl KUBECONFIG 경로(cygpath, PR #181).
- **#174 ES nori 환경 검증**: ES green·analysis-nori 로드·es-reindex 정상(PR #177/#179, shared#53 ECR nori 이미지).
- **양 환경 Healthy 달성**: 앱 ECR 이미지 0개→앱팀 deploy 트리거로 빌드·push, staging 태그 정정(#191/PR #192) → dev 9/9·staging 7/7. ArgoCD 16/16 Healthy 기준점으로 **24h 소크(NFR-305) 진입**(#183 결정: 클러스터 유지).
- destroy 행 근본원인(orphan ALB/ENI) 선정리 자동화(PR #178). EKS 컨트롤플레인 알람 false-positive 비활성(#194/PR #195).

### 의사결정
- **engagement CrashLoop 오진→원복 교훈**: #164 패턴으로 추측 수정(#187)→무효→원복(#188). `describe` 증거(Liveness 401·exitCode 143)로 실원인=앱 actuator 401 확정 → 앱레포 engagement-svc#43 위임. **증상→수정 직행 금지, 증거로만**.
- teardown 보류(#183): Day4 검증(staging·Observability·24h·nori)용 유지, 소크 후 destroy.

### 이벤트
- bastion IAM으로 bring-up 자동화 중단 → team-lead 수동 우회(#182 등록). 서비스 ECR 레포 7종 부재 → 수동 선생성.

### 산출물
- PR #176~#192(핸드오프·ES·destroy 선정리·태그 정정·IU bump). `HANDOFF_2026-06-11.md`.

---

## 2026-06-12 (라이브 윈도우) — #174 인증 검색 E2E 완주(3겹 차단 해소) · #164/#194 close

### 무엇을 했는지
- **#174 완주·close**: 게이트웨이 토큰 풀 플로우(signup 201→login→노트 생성 200→검색 200+nori 형태소 하이라이트, P3 0.1~0.18s). 도달까지 3겹 차단 해소:
  - **#196/PR #197**: 인증 API 전면 401 — gateway SM 공개키가 06-08 시드 때 오염(modulus 대조 확정) + knowledge·engagement·learning-card JWT_PUBLIC_KEY 미주입(ephemeral 폴백) → 4앱 platform 공개키 단일 출처화.
  - **#198/PR #200**: 검색 인덱싱 3중 단절 — search-sync 토픽(+.dlq) 정본 누락·knowledge Redis 미설정(전 레코드 DLQ행, DLQ 헤더로 확정)·SEARCH_AI_BASE_URL 폴백.
  - **#199(신규 OPEN)**: dev/staging 동일 MSK·동일 컨슈머 그룹 공유 → 파티션 환경교차 분산(dev 이벤트 2/3이 staging행). 팀 결정 필요.
- **#194 close**: SynapseHighMemory 12건 = `container!=""` 부재 이중합산 오탐(사용률 2배). 룰 수정 + platform-svc 768Mi(12h P95 96.7% 실측, P95×1.3, PR #201) → 신규 파드 52~53%.
- **#164 close**: 양 환경 18h+ RESTARTS 0 재확인. 잔여 401은 learning-svc#74.
- **#182 진행(PR #202)**: 진단 정정(IAM 인라인 정책 현존, 실원인=cloud-init IAM 전파 레이스→depends_on+재시도) + **ECR 8종 standalone 스택**(`infra/aws/ecr/`, 자체 state·prevent_destroy) import/apply 완료.

### 의사결정
- **JWT 공개키 단일 출처(서명 주체 platform 경로)**: gateway 전용 사본 키가 드리프트 사고의 원인 → 사본 경로 폐기로 구조적 재발 차단.
- **ECR을 dev 스택과 분리**: dev 스택 포함 시 윈도우 destroy마다 이미지 전부 삭제 → 별도 state로 윈도우 수명주기와 절연.
- staging 파드 재기동 최소화(24h 소크 보호): platform-svc만 ArgoCD 자동 sync로 롤링(불가피·#183 고지), knowledge staging 신규 설정은 다음 재기동 시 적용.

### 이벤트
- 게이트웨이 경유 인증이 이번에 처음 실검증됨 — 기존 검증은 전부 in-cluster 직접 호출이라 게이트웨이 결함 미노출.

### 산출물
- PR #197·#200·#201·#202·#203(핸드오프). 이슈 #196·#198 close, #199 신규. `HANDOFF_2026-06-12.md`. destroy는 17:15 소크 사인오프 후.

---

## 2026-06-21 (Final Phase D) — GitOps/release hardening 정렬

### 무엇을 했는지
- 최종 리팩토링 완료 계획의 Phase D 기준으로 dev Image Updater 계약과 dev overlay 태그를 재정렬했다. dev ApplicationSet은 semver 전략이므로 frontend/gateway/platform/knowledge/learning-ai/learning-card dev 태그를 `1.0.0` semver baseline으로 되돌리고 engagement는 기존 `1.0.1` semver를 유지했다.
- W5 Step 12 runbook 기준에 맞춰 prod 5개 서비스 PDB를 `minAvailable: 2`로 정렬했다.
- standalone ECR Terraform에 `Environment=shared`, `Service=<svc>` 비용 태그를 추가했다.
- `scripts/verify-phase-d-release-hardening.ps1`와 Phase D runbook을 추가해 image tag 계약, Image Updater write-back 계약, HPA/PDB, 비용 태그를 로컬에서 반복 검증할 수 있게 했다.

### 의사결정
- **dev Image Updater는 semver 유지**: 현재 ApplicationSet이 `update-strategy: semver`와 semver allow-tags를 쓰므로 SHA 태그를 그대로 두면 Image Updater가 skip한다. Phase D의 semver/write-back 기준을 우선해 dev overlay 태그를 semver로 정렬했다.
- **prod 승격은 수동 PR 유지**: prod ApplicationSet에 Image Updater annotation을 추가하지 않았다. 기존 W4 승인 게이트와 명시적 prod promotion 정책을 유지한다.

### 이벤트
- 로컬 환경에 `kustomize`/`yamllint`/`kubeconform`이 없어 렌더/스키마 검증은 실행하지 못했다. 대신 Phase D 계약 스크립트는 통과했다. 도구가 있는 환경에서는 `.\scripts\verify-phase-d-release-hardening.ps1 -RunKustomize`로 overlay build까지 확인한다.
