...
**작업**: 5개 prod kustomization `REPLACE_ME_*` 치환 + `SPRING_DATA_REDIS_DATABASE=1` 추가 + ExternalSecret `synapse/prod/{app}/*` 패치. replicas=3·profile=prod는 스캐폴드 완료.
...
- `gitops-admin` → prod-deployer → `synapse-prod/*` sync **허용**
...
|---|---|---|---|
| FR-GO-401 prod overlay ×5 | P0 | 5개 kustomization `REPLACE_ME` 해소 + kustomize build | 0 |
